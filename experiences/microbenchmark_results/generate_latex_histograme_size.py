...
def generate_latex(deg_files, java_files):
    try:
        with open(deg_files[0], 'r') as df1, open(deg_files[1], 'r') as df2, open(deg_files[2], 'r') as df3, open(java_files[0], 'r') as jf1, open(java_files[1], 'r') as jf2, open(java_files[2], 'r') as jf3, open("sizes.tex", 'w') as out:
            ...
            out.write("    legend style={draw=none, at={(0.8,1.05)}, anchor=north, font=\\footnotesize}},\n")
            ...
            out.write("    y tick label style={font=\\footnotesize},\n")
            out.write("    x tick label style={font=\\footnotesize},\n")
            out.write("]\n")
            ...
            for line1, line2, line3 in zip(df1, df2, df3):
                ...
                out.write("\\addplot+[nodes near coords, point meta=explicit symbolic, every node near coord/.append style={yshift=6pt, xshift=-0.1cm, rotate=45, anchor=west, font=\\footnotesize}, error bars/.cd, y dir=both, y explicit] coordinates {\n (16k," + str(int_mean[0]) + ") +- ("+str(lower_margin[0])+", "+str(upper_margin[0])+") \n (32k," + str(int_mean[1]) + ") +- ("+str(lower_margin[1])+", "+str(upper_margin[1])+") \n (65k," + str(int_mean[2]) + ") +- ("+str(lower_margin[2])+", "+str(upper_margin[2])+") \n};\n")
            ...
            out.write("    xtick={0,1,2},\n")
            out.write("    xticklabels={16K,32K,64K},\n")
            out.write("    legend style={draw=none, at={(0.785,0.88)}, anchor=north, font=\\footnotesize}},\n")
            out.write("    enlarge x limits=0.3,\n")
            out.write("    cycle list={ {fill=blue}, {fill=blue} },\n")
            out.write("    area legend,\n")
            out.write("    ylabel style={font=\\footnotesize},\n")
            out.write("    xlabel style={font=\\footnotesize},\n")
            out.write("    title style={font=\\footnotesize},\n")
            ...
            for line1, line2, line3 in zip(jf1, jf2, jf3):
                ...
                out.write("\\addplot+[nodes near coords, point meta=explicit symbolic, every node near coord/.append style={yshift=3pt, xshift=-0.35cm, rotate=45, anchor=west, font=\\footnotesize}, error bars/.cd, y dir=both, y explicit]coordinates {\n (16k," + str(int_mean[0]) + ") +- ("+str(lower_margin[0])+", "+str(upper_margin[0])+") \n (32k," + str(int_mean[1]) + ") +-("+str(lower_margin[1])+", "+str(upper_margin[1])+") \n (65k," + str(int_mean[2]) + ") +-("+str(lower_margin[2])+", "+str(upper_margin[2])+") \n};\n")
